...
function generateResume(format, resumeData) {
 ...
 */
function generateIITBombayFormat(data) {
    const resume = [];
    // Header
    resume.push("═══════════════════════════════════════════════════════════════");
    ...
    if (data.skills && data.skills.length > 0) {
        ...
        resume.push(data.skills.map((skill) => (typeof skill === "string" ? skill : skill.name)).join(" • "));
        resume.push("");
    }
    // Projects
 ...
 */
function generateOverleafLatexFormat(data) {
    const resume = [];
    ...
    if (data.skills && data.skills.length > 0) {
        ...
        const skillsText = data.skills
            .map((skill) => escapeLatex(typeof skill === "string" ? skill : skill.name))
            .join(" $\\cdot$ ");
